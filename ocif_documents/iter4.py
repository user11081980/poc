...
class WebScraper:
    ...
    def __init__(
        ...
        # URL configuration
        self.start_urls = [self._canonicalize_url(url) for url in (start_urls or [])]
        ...
        self.document_extensions = [
            ext.lower()
            for ext in (document_extensions or [
                ".pdf",
                ".doc",
                ".docx",
                ".xls",
    ...
    def _is_document_url(self, url: str) -> bool:
        ...
        return any(lower_url.endswith(ext) for ext in self.document_extensions)
    ...
    def _normalize_link(self, base_url: str, href: str) -> str:
        ...
        return self._canonicalize_url(absolute)
    ...
    def _extract_title_from_url(self, url: str) -> str:
        """Extract filename from URL, remove extension, and URL-decode."""
        parsed = urlparse(url)
        path = parsed.path
        ...
        decoded_title = unquote(filename_without_ext)
                ...
                while queue and len(results) < self.max_pages:
                    ...
                    try:
                        ...
                        if page_content:
                            ...
                            output_data["document_links"] = page_content.get("document_links", [])
    ...
    async def _scrape_page(self, context: BrowserContext, url: str, index: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Extract links for crawling and document discovery
            links = []
            document_links = []
            href_items = await page.eval_on_selector_all(
                "a[href]",
                "elements => elements.map(e => ({ href: e.href, text: e.textContent }))"
            )
            for item in href_items:
                href = item.get("href") or ""
                title_text = (item.get("text") or "").strip()
                if not href:
                    continue

                normalized_url = self._normalize_link(url, href)
                if self._is_document_url(normalized_url):
                    if not title_text or title_text == "Text Link" or title_text == "": # modified
                        title_text = self._extract_title_from_url(normalized_url)
                    document_links.append({
                        "title": title_text or normalized_url,
                        "url": normalized_url,
                    })
                    continue

                if self.follow_links and normalized_url.startswith("http") and self._is_same_domain(normalized_url):
...
async def main():
    ...
    seen_urls = set()
    for page in results:
        for doc in page.get("document_links", []):
    ...
    document_output_file = "document_links.json"
    with open(document_output_file, "w", encoding="utf-8") as f:
        json.dump(document_links, f, indent=2, ensure_ascii=False)
    
    logger.info(f"Results saved to {output_file}")
    logger.info(f"Document links saved to {document_output_file}")
    print(f"\nScraped {len(results)} pages.")
    print(f"Results saved to: {output_file}")
    print(f"Document links saved to: {document_output_file}")
    
    # Print first document link as sample
    if document_links:
        print("\nSample document link:")
        print(json.dumps(document_links[0], indent=2, ensure_ascii=False))
